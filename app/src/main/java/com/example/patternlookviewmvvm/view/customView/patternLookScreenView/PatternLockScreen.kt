package com.example.patternlookviewmvvm.view.customView.patternLookScreenView
...
import com.example.patternlookviewmvvm.view.customView.dotView.DotView
import com.example.patternlookviewmvvm.view.customView.model.Dot

class PatternLockScreen @JvmOverloads constructor(
    context: Context,
    ...
    override fun onDraw(canvas: Canvas?) {
        ...
        if (state is PatternViewState.Error || state is PatternViewState.Success) {
    ...
    private fun drawPatternView(
        ...
        for (rowIndex in 0 until rowSize) {
            createRow(this@PatternLockScreen, layoutParams).apply {
    ...
    fun getPassword() = markedDotList.map { it.key }.joinToString("")
